  ...
  <div class="app-container home" v-else>
    <el-card>
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center;">
          <div style="display: flex; align-items: center; gap: 15px;">
            <span style="font-weight: bold; font-size: 16px;">排班与课表信息</span>
            <el-select v-model="selectedWeek" placeholder="选择周次" style="width: 150px">
              <el-option v-for="week in 17" :key="week" :label="`第 ${week} 周`" :value="week"></el-option>
            </el-select>
          </div>
          <div style="display: flex; align-items: center; gap: 12px;">
            <el-button type="success" @click="router.push('/student/attendance/index')">前往考勤管理</el-button>
            <el-upload
              action="#"
              :auto-upload="false"
            ...
            >
              <el-button type="primary" icon="Upload" :loading="uploadingSchedule">上传课表照片</el-button>
            </el-upload>
          </div>
        </div>
      </template>
      ...
      >
        <template #title>
          课表上传成功：
          <el-link :href="lastUploadedScheduleUrl" target="_blank" type="primary">查看原图</el-link>
          <span v-if="lastScheduleSyncAt" style="margin-left: 12px; color: #606266;">最近同步：{{ lastScheduleSyncAt }}</span>
        </template>
      </el-alert>
      <el-table :data="scheduleData" style="width: 100%" border stripe :cell-style="{ height: '64px' }">
        <el-table-column prop="time" label="值班时间" width="150" align="center" fixed />
        <el-table-column prop="mon" label="星期一" min-width="140" align="center" />
        <el-table-column prop="tue" label="星期二" min-width="140" align="center" />
        <el-table-column prop="wed" label="星期三" min-width="140" align="center" />
        <el-table-column prop="thu" label="星期四" min-width="140" align="center" />
        <el-table-column prop="fri" label="星期五" min-width="140" align="center" />
        <el-table-column prop="sat" label="星期六" min-width="140" align="center" />
        <el-table-column prop="sun" label="星期日" min-width="140" align="center" />
        </el-table>
    </el-card>
  </div>
</template>
...
const isStudent = computed(() => {
  if (hasStudentRoutes(permissionStore.sidebarRouters)) {
    return true
  }
...
function hasStudentRoutes(routes) {
  ...
  for (const route of list) {
    ...
    if (hasStudentRoutes(route?.children)) {
      return true
    }
  }
  return false
...
// Schedule data logic
const selectedWeek = ref(1) // 默认选择第1周
...
function resolveUploadResult(response) {
  ...
  if (typeof data === "string") {
    ...
    parsedSchedule = data.parsedSchedule || data.scheduleData || data.freeSchedule || null
  }
  if (!parsedSchedule) {
...
function applyParsedSchedule(parsedSchedule) {
  ...
  scheduleData.value = DUTY_PERIODS.map((time) => rowsByTime.get(normalizeTimeLabel(time)) || { time, ...emptyScheduleRow() })
