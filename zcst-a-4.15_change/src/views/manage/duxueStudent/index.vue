    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['manage:duxueStudent:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:duxueStudent:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:duxueStudent:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['manage:duxueStudent:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
...
<script setup name="DuxueStudent">
import {
  listStudent,
  getStudent,
  delStudent,
...
function getList() {
  ...
  listStudent(params).then((response) => {
    duxueStudentList.value = response.rows;
...
function reset() {
  form.value = {
    ...
    grade: null,
    phone: null,
    venueId: userStore.venueId || 4, // 默认场馆为笃学馆
  };
  proxy.resetForm("duxueStudentRef");
}
...
function handleUpdate(row) {
  ...
  getStudent(_studentId).then((response) => {
    ...
    open.value = true;
    title.value = "修改笃学馆学生管理";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["duxueStudentRef"].validate((valid) => {
    if (valid) {
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除笃学馆学生管理编号为"' + _studentIds + '"的数据项？')
    .then(function () {
...
function handleExport() {
  proxy.download(
    "manage/duxueStudent/export",
    {
      ...queryParams.value,
    },
    `duxueStudent_${new Date().getTime()}.xlsx`,
  );
}
