    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['manage:xinyuanStudent:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:xinyuanStudent:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:xinyuanStudent:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['manage:xinyuanStudent:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
...
<script setup name="XinyuanStudent">
import {
  listStudent,
  getStudent,
  delStudent,
  addStudent,
  updateStudent,
} from "@/api/manage/xinyuanStudent";
...
function getList() {
  ...
  listStudent(params).then((response) => {
    xinyuanStudentList.value = response.rows;
...
function reset() {
  form.value = {
    ...
    grade: null,
    phone: null,
    venueId: userStore.venueId || 3, // 默认场馆为心缘馆
  };
  proxy.resetForm("xinyuanStudentRef");
}
...
function handleUpdate(row) {
  ...
  getStudent(_studentId).then((response) => {
    ...
    open.value = true;
    title.value = "修改心缘馆学生管理";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["xinyuanStudentRef"].validate((valid) => {
    if (valid) {
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除心缘馆学生管理编号为"' + _studentIds + '"的数据项？')
    .then(function () {
...
function handleExport() {
  proxy.download(
    "manage/xinyuanStudent/export",
    {
      ...queryParams.value,
    },
    `xinyuanStudent_${new Date().getTime()}.xlsx`,
  )
}
