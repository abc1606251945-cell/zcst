              ...
              >
                <div v-if="getCellMeta(day.dateStr, slotIndex).items.length" class="event-layer">
                  ...
                  >
                    ...
                    <div class="event-building">{{ formatBuildingLocation(item) }}</div>
                    <div class="event-time">{{ item.timeLabel || "-" }}</div>
                  </div>
                </div>
...
const COURSE_THEME_PALETTE = [
  {
    background: "rgba(179, 229, 252, 0.2)", // 浅蓝
    border: "rgba(179, 229, 252, 0.55)",
  },
  {
    background: "rgba(200, 230, 201, 0.2)", // 浅绿
    border: "rgba(200, 230, 201, 0.55)",
  },
  {
    background: "rgba(225, 190, 231, 0.2)", // 浅紫
    border: "rgba(225, 190, 231, 0.55)",
  },
]
...
function getCourseTheme(item) {
  const key = `${item?.dutyId || ""}-${item?.title || ""}-${item?.dateStr || ""}`
  const index = (hashString(key) + colorSeed) % COURSE_THEME_PALETTE.length
  return COURSE_THEME_PALETTE[index]
}
...
function getEventStyle(item, slotIndex, index) {
  ...
  const top = (topOffset / 60) * ROW_HEIGHT + 6
  const height = (extendedDuration / 60) * ROW_HEIGHT - 12
  const style = {
    top: `${top}px`,
...
function normalizeWeekSchedule(list) {
  ...
  for (const raw of list || []) {
    ...
    normalized.push({
      ...
      type: "course",
      title,
      building: String(raw.building || raw.lh || "").trim() || location,
      location,
      timeLabel: buildTimeLabel(startMin, endMin),
      dateStr,
      startMin,
...
function parseScheduleCell(value) {
  ...
  if (typeof value === "object") {
    ...
    const building = String(value.building || value.lh || "").trim()
    ...
    return { title, building: building || location || "地点待定", location: location || "地点待定" }
  }
  ...
  return { title, building: location, location }
}

function formatBuildingLocation(item) {
  ...
  if (building && location) {
    const room = location.replace(building, "").replace(/^[\s\-_/：:]+/, "").trim()
  ...
  for (const row of props.scheduleRows || []) {
    ...
    for (const dayKey of weekdayKeys) {
      ...
      list.push({
        ...
        type: "course",
        title: parsed.title,
        building: parsed.building || parsed.location || "地点待定",
