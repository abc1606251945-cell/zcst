    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          icon="Plus"
          @click="handleAdd"
          v-hasPermi="['manage:zhixingStudent:add']"
          >新增</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['manage:zhixingStudent:edit']"
          >修改</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['manage:zhixingStudent:remove']"
          >删除</el-button
        >
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          icon="Download"
          @click="handleExport"
          v-hasPermi="['manage:zhixingStudent:export']"
          >导出</el-button
        >
      </el-col>
      <right-toolbar
...
<script setup name="ZhixingStudent">
import {
  listStudent,
  getStudent,
  delStudent,
  addStudent,
  updateStudent,
} from "@/api/manage/zhixingStudent";
...
function getList() {
  ...
  listStudent(params).then((response) => {
    zhixingStudentList.value = response.rows;
...
function reset() {
  form.value = {
    ...
    grade: null,
    phone: null,
    venueId: userStore.venueId || 5, // 默认场馆为知行馆
  };
  proxy.resetForm("zhixingStudentRef");
}
...
function handleUpdate(row) {
  ...
  getStudent(_studentId).then((response) => {
    ...
    open.value = true;
    title.value = "修改知行馆学生管理";
  });
}

/** 提交按钮 */
function submitForm() {
  proxy.$refs["zhixingStudentRef"].validate((valid) => {
    if (valid) {
...
function handleDelete(row) {
  ...
  proxy.$modal
    .confirm('是否确认删除知行馆学生管理编号为"' + _studentIds + '"的数据项？')
    .then(function () {
...
function handleExport() {
  proxy.download(
    "manage/zhixingStudent/export",
    {
      ...queryParams.value,
    },
    `zhixingStudent_${new Date().getTime()}.xlsx`,
  );
}
